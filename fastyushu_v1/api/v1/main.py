from fastapi import APIRouter, Depends, Request, status, HTTPException, Query, Header
...
@application.post("/read_user")#查询用户
async def create_user(user: str, db: Session = Depends(get_db)):
    user_data = jwt_get_user(db=db,username = user)
    if user_data:
        return user_data
    
@application.post("/create_group",response_model = schemas.ReadGroup)#创建角色
async def create_group(request: Request, group: schemas.CreateGroup, db: Session = Depends(get_db), current_user: User = Depends(verify_token)):
...
@application.get("/read_group")
async def raadgroup(groupname: Optional[str] = None, current_user: User = Depends(verify_token), db: Session = Depends(get_db)):
    results = crud.get_group(db=db,groupname=groupname)
    if not results:
...
# @application.post("/test_token")
# async def test_token(access_token:str = Header(...)):
#     if access_token:
#         return access_token
#     else:
#         raise HTTPException(
...
@application.get("/jwt/users/me1",response_model = schemas.ReadUser)#测试通过x-token获取用户信息
async def jwt_read_users_me11(user: str = Depends(verify_token)):
    return user
