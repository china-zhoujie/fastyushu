...
#设置不需要验证token的接口
No_verify_token = ["token","create_user"]
...
def jwt_get_user(username: str, db: Session = Depends(get_db)):
    ...
    if user:
        ...
        print(group_detail.UserPrivileges)
        if group_detail:
...
def jwt_get_current_user(token: str, db:Session = Depends(get_db)):#获取当前用户
